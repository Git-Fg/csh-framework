...
### JSON-First Principle (`--json`)

Agents handle structured JSON significantly better than unstructured text. Your primitives SHOULD provide a `--json` or `--output json` flag.

```bash
mytool list plugins --json
```
