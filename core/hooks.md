...
## Mandatory Language: When to Use Strong Directives

AI agents often ignore gentle suggestions like "Consider using X". Hooks need strong language to cut through and achieve actual compliance.

### The Difference: Prescriptive vs Mandatory

| Concept | Description | Example |
|---------|-------------|---------|
| **Prescriptive** | Telling AI what CORE TASK to do (wrong) | "You must use Python for this" |
| **Mandatory** | Using strong language WHEN guiding AI to use right tools (good) | "YOU MUST use vault-search skill" |

### When to Use Mandatory Language

Use **"YOU MUST"** or **"YOU SHOULD"** in hooks when:

| Situation | Example | Why |
|-----------|---------|-----|
| **AI uses wrong tool** | after_tool_call detects `memory_search` → "YOU SHOULD use clawvault-search" | Redirect to better approach |
| **Safety at risk** | "YOU MUST verify backup before deployment" | Prevent data loss |
| **Detected intent** | llm_output detects "remember this" → "YOU MUST use remember skill" | Capture important context |
| **Critical workflow** | before_compaction → "YOU MUST save context to vault" | Preserve important data |
| **Convention enforcement** | "YOU MUST use dry-run before production" | Enforce project standards |

### When to Use Advisory Language

Use gentler language when:

| Situation | Example | Why |
|-----------|---------|-----|
| **Multiple valid paths** | "You can also use X" | Preserve agent autonomy |
| **Discovery phase** | "Available skills: a, b, c" | Pure information |
| **Exploration** | "TIP: Try X for better results" | Suggest without forcing |

### Decision Matrix

```
IF safety/data at risk → "YOU MUST"
ELSE IF AI using wrong tool → "YOU SHOULD"
ELSE IF detected intent (remember, save) → "YOU MUST"
ELSE → Advisory ("Consider", "You may")
```

### Examples from ClawVault

```bash
# After AI used memory_search (wrong tool)
echo "[ClawVault: For deeper semantic search, YOU SHOULD ALSO use clawvault-search skill.]"

# After AI mentioned "remember this"
echo "[ClawVault MANDATORY: You detected worth-saving context. YOU MUST use clawvault-remember skill.]"

# Before memory compaction
echo "[ClawVault MANDATORY: Compaction pending. YOU MUST save important context to vault.]"
```
