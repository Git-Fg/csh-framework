...
**Symptom**: Plugin installed but not showing in `openclaw plugins list`

**Cause**: `plugins.allow` is configured as an allowlist

**Diagnosis**:
```bash
# Check if plugins.allow is set
cat ~/.openclaw/openclaw.json | jq '.plugins.allow'
```

**Solution**:

**If `plugins.allow` is empty or null** → Plugins auto-discover:
```json
"plugins": {
  "allow": []  // Empty = auto-discover all found plugins
}
```

**If `plugins.allow` has entries** → Only those plugins load:
```json
"plugins": {
  "allow": ["searxng", "voice-transcribe"]  // Only these load!
}
```

**Fix**: Add your plugin to the allowlist:
```bash
# Option 1: Add via CLI
openclaw plugins enable my-plugin

# Option 2: Edit config manually
openclaw config set plugins.allow '["existing-plugin", "my-plugin"]'
```
