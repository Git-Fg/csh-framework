...
**Dev-First Installation Flow:**

```bash
# === FOR DEVELOPERS (Recommended) ===
# 1. Link your CLI locally for development
...
**Why `-l` for development?**
- Creates a **symlink** (not copy) → edits to plugin files take effect immediately
- **Hot reload** for hooks & plugin code - no gateway restart needed
- **Iterate faster** - change → test → change

**⚠️ Hot Reload Behavior:**

| Component | Hot Reload? | Requires Restart? |
|-----------|-------------|-------------------|
| Hooks (api.on) | ✅ Yes | No |
| Plugin code (src/index.ts) | ✅ Yes | No |
| Skills (SKILL.md) | ❌ No | Yes (cached at startup) |

**What's installed automatically:**
- ✅ **Skills**: Auto-discovered from plugin's `skills/` directory
- ✅ **Hooks**: Auto-loaded from plugin's `hooks/` directory
- ✅ **CLI integration**: If plugin wraps a CLI tool

**⚠️ Important: plugins.allow**

If your plugin doesn't load after install, check the allowlist:

```bash
# Check current allowlist
cat ~/.openclaw/openclaw.json | jq '.plugins.allow'

# If it has entries, you must add your plugin
# Empty = auto-discover (all plugins load)
# Has entries = only those plugins load
```
