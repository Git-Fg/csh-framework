...
# 3. Edit code → changes auto-reload (no restart needed!)
#    Test immediately with: openclaw chat -m "test your plugin"
```

**Why `-l`?**
- Creates **symlink** → edits to plugin files take effect immediately
...
openclaw plugins install /path/to/clawvault-extension
...
### ⚠️ Plugin Not Loading?

If your plugin doesn't appear after install, check `plugins.allow`:
...
**Fix**: Add your plugin to allowlist:
```bash
